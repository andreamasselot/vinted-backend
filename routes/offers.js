...
router.post("/offer/publish", isAuthenticated, fileUpload(), async (req, res) => {
  try {
    ...
    const result = await cloudinary.uploader.upload(convertToBase64(imageToUpload));
...
router.get("/offers", isAuthenticated, async (req, res) => {  //route authentifiée 
  ...
  try {
    ...
    } if (priceMax) { priceFilter.$lte = priceMax }; // le filtre ne doit pas s'appliquer si on ne le selectionne pas ( => condition if pricemin existe ou pas)
    const sortFilter = {}
    if (sort === "price-desc") {  //ajouter le product_price a l'objet sortFilter
      sortFilter.product_price = -1  //product_price trouvé car défini dans newOffer puis saved
    } else if (sort === "price-asc") { //définir le filtre de prix pour que les query correspondent avec "price-desc" et "price-asc"
    ...
    const offers = await Offer.find({ "product_name": regExp, "product_price": priceFilter }).sort(sortFilter).skip(currentPage).limit(3).populate("owner", "account"); // récupère toutes les annonces + applique lles filtres
